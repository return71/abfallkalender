...
class KalenderParser():
    ...
    def __init__(self, seite):
        ...
        self.Muelltonnen = {'tonne_schwarz': 'Muell',
                            ...
                            'tonne_schwarz_blau_gelb_braun': 'Muell, Bio,\
Plaste, Papier',
        ...
        for tabelle in range(1, 13):
            ...
            for zeile in range(2, 8):
                for spalte in range(1, 8):
                    s = '/html/body/div/div[%s]/table/tr[%s]/td[%s]' % \
...
#                    print(element)
                    if len(element) > 0:
                        ...
                        if len(element_text) > 0:
                            tagcounter += 1
#                            print(element.text.lstrip().rstrip(), tabelle,\
#                                  zeile, spalte, tagcounter)
                        if len(element) > 0:
                            tagcounter += 1
...
#                            datum = str(tagcounter) + '.' + str(tabelle) + \
#                                     '.' + str(year)
